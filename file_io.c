...
void read_camera_data(FILE* file_to_read, Shape *camera) {
	...
	char wastebasket[10]; // initialize a junk data variable
  fscanf(file_to_read, "%s", wastebasket); // read past the camera identifier
	...
	fscanf(file_to_read, "%lf", &camera->camera.width); // capture the width value
	...
	fscanf(file_to_read, "%lf", &camera->camera.height); // capture the height value
  /*
	printf("%d\n", camera->type);
	printf("%lf\n", camera->camera.width);
...
void read_sphere_data(FILE* file_to_read, Shape* output_list, int obj_index) {
	...
	fscanf(file_to_read, "%lf", &output_list[obj_index]->sphere.color->x); // read in red color value
	fgetc(file_to_read); // skip over comma
	traverse_whitespace_and_comments(file_to_read); // skip over spaces
	fscanf(file_to_read, "%lf", &shapes_list[obj_index]->sphere.color->y); // read in green color value
	fgetc(file_to_read); // skip over comma
	traverse_whitespace_and_comments(file_to_read); // skip over spaces
	fscanf(file_to_read, "%lf", &shapes_list[obj_index]->sphere.color->z); // read in blue color value
